package content.global.skill.free.woodcutting

import config.Items
import config.NPCs
import config.Sounds
import config.Sounds.TREE_FALLING_2734
...
import content.global.skill.member.farming.FarmingPatch.Companion.forObject
import content.global.skill.skillcape.perks.SkillcapePerks
import content.global.skill.skillcape.perks.SkillcapePerks.Companion.isActive
...
class WoodcuttingListener : InteractionListener {
    ...
    fun checkWoodcuttingRequirements(player: Player, resource: WoodcuttingNode, node: Node): Boolean {
        ...
        if (player.inventory.freeSlots() < 1 && node.isActive) {
            sendMessage(player, "Your inventory is too full to hold any more " + ItemDefinition.forId(resource.getReward()).name.lowercase(Locale.getDefault()) + ".")
            return false
        }
