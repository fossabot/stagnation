...
import content.global.skill.free.magic.spellconsts.Modern
import content.global.skill.free.prayer.Bones
import content.global.skill.free.smithing.smelting.Bar
import content.global.skill.free.smithing.smelting.SmeltingPulse
...
class ModernListeners : SpellListener("modern"){
    ...
    private val LOW_ALCH_GFX = Graphics(config.Graphics.LOW_ALCH_112,5)
    private val HIGH_ALCH_ANIM = Animation(Animations.HUMAN_CAST_HIGH_ALCHEMY_SPELL_713)
    private val HIGH_ALCH_GFX = Graphics(config.Graphics.HIGH_ALCH_113,5)
    ...
    override fun defineListeners() {
        ...
        onCast(Modern.APE_ATOLL_TELEPORT, NONE){ player, _ ->
            requires(player,64, arrayOf(Item(Items.FIRE_RUNE_554,2),Item(Items.WATER_RUNE_555,2),Item(Items.LAW_RUNE_563,2),Item(
                Items.BANANA_1963)))
        ...
        for(item in player.inventory.toArray()){
            ...
            if(isInMTA){
                if(bones.contains(item.id)){
                    ...
                    val amount = inInventory * (content.minigame.magetrainingarea.impl.GraveyardZone.BoneType.forItem(Item(item.id)).ordinal + 1)
                    if(amount > 0){
    ...
    private fun superheat(player: Player,item: Item){
        ...
        if(bar == Bar.IRON && player.inventory.getAmount(Items.COAL_453) >= 2 && player.skills.getLevel(Skills.SMITHING) >= Bar.STEEL.level && inInventory(player, 
                Items.IRON_ORE_441,1)) bar = Bar.STEEL
    ...
    public fun alchemize(player: Player, item: Item, high: Boolean) : Boolean {
        ...
        if (anyInEquipment(player, *fireStuff)) {
            visualize(player,
                if (high) Animations.HUMAN_CAST_HIGH_ALCH_SPELL_WITH_STAFF_9633 else Animations.HUMAN_CAST_LOW_ALCH_SPELL_WITH_STAFF_9625,
                if (high) config.Graphics.HIGH_ALCH_WITH_STAFF_1693 else config.Graphics.LOW_ALCH_WITH_STAFF_1692)
        } else {
            visualize(player,
                if (high) Animations.HUMAN_CAST_HIGH_ALCHEMY_SPELL_713 else Animations.HUMAN_CAST_LOW_ALCHEMY_SPELL_712,
                if (high) config.Graphics.HIGH_ALCH_113 else config.Graphics.LOW_ALCH_112)
        }
