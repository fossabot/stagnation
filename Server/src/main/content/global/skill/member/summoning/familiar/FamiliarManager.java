...
import content.global.skill.member.summoning.SummoningPouch;
import content.global.skill.member.summoning.pet.Pet;
import content.global.skill.member.summoning.pet.PetDetails;
import content.global.skill.member.summoning.pet.Pets;
...
public final class FamiliarManager {
	...
	public void summon(Item item, boolean pet, boolean deleteItem) {
		...
		if (fam == null) {
			player.getPacketDispatch().sendMessage("Invalid familiar " + npcId + " - report on gitlab");
			return;
		}
        if(!renew) {
