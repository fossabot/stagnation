...
import content.global.skill.member.hunter.HunterManager
import core.api.*
...
class HunterPitfall : OptionHandler() {
    ...
    override fun handle(player: Player?, node: Node?, option: String?): Boolean {
        node ?: return true
        player ?: return true
        sendMessage(player, "Hello from HunterPitfall: ${option}")
        when(option) {
            "tease" -> {
    ...
    override fun defineListeners() {
            ...
            if(dir != null) {
                ...
                if(pitfall_npc != null && pitfall_npc.getLocation().getDistance(src) < 3.0) {
                    ...
                    if(last_pit_loc == pit.location) {
                        sendMessage(player, "The ${pitfall_npc.name.toLowerCase()} won't jump the same pit twice in a row.")
                        return@on true
                    }
        ...
        on(BEAST_IDS, IntType.NPC, "tease") { player, node ->
            ...
            if(player.skills.getLevel(Skills.HUNTER) < hunterReq) {
                sendMessage(player, "You need a hunter level of ${hunterReq} to hunt ${entity.name.toLowerCase()}s.")
                return@on true
            }
            if(!player.inventory.containsItem(TEASING_STICK)) {
                sendMessage(player, "You need a teasing stick to hunt ${entity.name.toLowerCase()}s.")
                return@on true
            }
