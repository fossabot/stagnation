package content.global.skill.skillcape.perks

import config.Items
import content.global.skill.free.runecrafting.Altar
import content.global.skill.member.farming.PatchType
import content.global.skill.member.farming.Plantable
import content.global.skill.skillcape.Skillcape
import core.api.*
...
enum class SkillcapePerks(val attribute: String, val effect: ((Player) -> Unit)? = null) {
    ...
    LIBRARIAN_MAGUS("cape_perks:librarian-magus",{player ->
        ...
        if(player.getAttribute("cape_perks:librarian-magus-charges",2) > 0 || System.currentTimeMillis() > time){
            if(System.currentTimeMillis() > time) setAttribute(player, "/save:cape_perks:librarian-magus-charges",3)
            player.dialogueInterpreter.open(509871234)
            if(System.currentTimeMillis() > time)
                setAttribute(player, "/save:cape_perks:librarian-magus-timer",System.currentTimeMillis() + java.util.concurrent.TimeUnit.DAYS.toMillis(1))
        }
    }),
    ...
    ABYSS_WARPING("cape_perks:abyss_warp",{ player ->
        ...
        if(player.getAttribute("cape_perks:abyssal_warp",3) > 0 || System.currentTimeMillis() > time){
            if(System.currentTimeMillis() > time) setAttribute(player, "/save:cape_perks:abyssal_warp",3)
            player.dialogueInterpreter.open(509871233)
            if(System.currentTimeMillis() > time)
                setAttribute(player, "/save:cape_perks:abyssal_warp_timer",System.currentTimeMillis() + java.util.concurrent.TimeUnit.DAYS.toMillis(1))
        } else {
    ...
    SEED_ATTRACTION("cape_perks:seed_attract",{player ->
        ...
        if(System.currentTimeMillis() > time){
            ...
            setAttribute(player, "/save:cape_perks:seed_attract_timer",System.currentTimeMillis() + java.util.concurrent.TimeUnit.DAYS.toMillis(1))
        } else {
            player.dialogueInterpreter.sendDialogue("Your cape is still on cooldown.","Ready in " + java.util.concurrent.TimeUnit.MILLISECONDS.toMinutes(time - System.currentTimeMillis()) + " minutes.")
        }
    }),
