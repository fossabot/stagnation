package content.global.diary.area.lumbridge
...
import content.quest.free.dragonslayer.dialogue.DukeHoracioDSDialogue
import content.quest.free.therestlessghost.dialogue.FatherUhrneyRGDialogue
...
/**
 * Represents the Lumbridge achievement diaries.
 */
class LumbridgeAchievementDiary : DiaryEventHookBase(DiaryType.LUMBRIDGE) {
    ...
    companion object {
        ...
        private val DRAYNOR_MARKET = ZoneBorders(3074, 3245, 3086, 3255, 0)
        private val FRED_HOUSE = ZoneBorders(3184, 3270, 3192, 3275, 0)
    ...
    override fun onResourceProduced(player: Player, event: ResourceProducedEvent) {
        when (player.viewport.region.id) {
            12439 -> when (event.itemId) {
                Items.STEEL_LONGSWORD_1295 -> {
                    if (event.original == Items.STEEL_BAR_2353 &&
                        event.source.id == Scenery.ANVIL_2783
                    ) {
                        finishTask(
                            player,
                            DiaryLevel.MEDIUM,
            ...
            12850 -> when (event.itemId) {
                ...
                Items.SILVER_BAR_2355 -> {
                    if (event.original == Items.SILVER_ORE_442 &&
                        event.source.id == Scenery.FURNACE_36956
                    ) {
                        finishTask(
                            player,
    ...
    override fun onInteracted(player: Player, event: InteractionEvent) {
        when (player.viewport.region.id) {
            ...
            13899 -> {
                if (event.target.id == Scenery.ALTAR_2480 && event.option == "craft-rune") {
    ...
    override fun onDialogueOptionSelected(player: Player, event: DialogueOptionSelectionEvent) {
        when (event.dialogue) {
            ...
            is TownCrierDialogue -> {
                if (inBorders(player, DRAYNOR_MARKET) && event.currentStage == 71) {
                    finishTask(
                        player,
                        DiaryLevel.BEGINNER,
