package content.global.diary.area.falador

import config.Items
import config.NPCs
import config.Scenery
import content.global.handlers.iface.FairyRing
import content.global.skill.free.crafting.lightsources.LightSources
...
import core.game.node.entity.skill.Skills.THIEVING
import core.game.world.map.zone.ZoneBorders

/**
 * Represents the Falador achievement diaries.
 */
class FaladorAchievementDiary : DiaryEventHookBase(DiaryType.FALADOR) {
    ...
    override fun onInteracted(player: Player, event: InteractionEvent) {
        ...
        if (event.option == "pickpocket" && (event.target.id in FALADOR_GUARD && inBorders(player, FALADOR_GENERAL_AREA)) && hasLevelStat(player, THIEVING, 40)) {
            finishTask(
                player,
                DiaryLevel.MEDIUM,
    ...
    override fun onUsedWith(player: Player, event: UseWithEvent) {
        when {
            inBorders(player, PORT_SARIM_FLOWER_PATCH) -> {
                if (event.used == Items.SCARECROW_6059 && event.with == 7847 && hasLevelStat(player, FARMING, 23) && FarmingPatch.forObjectID(7840)?.getPatchFor(player)?.plantable == null) {
                    finishTask(
                        player,
                        DiaryLevel.MEDIUM,
