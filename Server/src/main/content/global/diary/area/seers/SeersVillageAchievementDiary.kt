package content.global.diary.area.seers
...
/**
 * Represents the Seers Village achievement diaries.
 */
class SeersVillageAchievementDiary : DiaryEventHookBase(DiaryType.SEERS_VILLAGE) {
    // https://www.youtube.com/watch?v=qxxhhCdxBsQ&t=75s
    companion object {
        ...
        private val CANDLE_SHOP_AREA = ZoneBorders(2791, 3432, 2802, 3442)
        private val TICKET_MERCHANT_SHOP = ZoneBorders(2660, 3434, 2660, 3425)
    ...
    override fun onItemPurchasedFromShop(player: Player, event: ItemShopPurchaseEvent) {
        if (inBorders(player, CANDLE_SHOP_AREA) &&
            inInventory(player, Items.COINS_995, 3) && freeSlots(player) < 1 &&
            (event.itemId == Items.CANDLE_36 || event.itemId == Items.BLACK_CANDLE_38)) {
            finishTask(
                player,
                DiaryLevel.EASY,
        ...
        if (inBorders(player, TICKET_MERCHANT_SHOP) &&
            amountInInventory(player, Items.ARCHERY_TICKET_1464) >= 100 &&
            hasLevelStat(player, Skills.RANGE, 40)) {

            if (event.itemId == Items.BARB_BOLTTIPS_47 && event.amount >= 10 ||         // Tickets: 50
                (event.itemId == Items.RUNE_ARROW_892 && event.amount >= 5) ||          // Tickets: 200
                (event.itemId == Items.GREEN_DHIDE_BODY_1135 && event.amount >= 1) ||   // Tickets: 2400
                (event.itemId == Items.ADAMANT_JAVELIN_829 && event.amount >= 1) ||     // Tickets: 100
                (event.itemId == Items.STUDDED_BODY_1133 && event.amount >= 1) ||       // Tickets: 150
                (event.itemId == Items.COIF_1169 && event.amount >= 1)                  // Tickets: 100
            )
                onButtonClicked(player, ButtonClickEvent(620, 23))
            finishTask(
                player,
                DiaryLevel.MEDIUM,
