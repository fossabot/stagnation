package content.global.diary.area.fremennik
...
import content.global.skill.free.cooking.dairy.DairyChurnDialogue
import content.minigame.barbassault.CaptainCainDialogue
import content.quest.member.thefremenniktrials.dialogue.ChieftanBrundtFTDialogue
...
/**
 * Represents the Fremennik achievement diaries.
 */
class FremennikAchievementDiary : DiaryEventHookBase(DiaryType.FREMENNIK) {
    ...
    override fun onInterfaceClosed(player: Player, event: InterfaceCloseEvent) {
        if (event.component.id == Components.YRSA_SHOE_STORE_200) {
            finishTask(
                player,
                DiaryLevel.MEDIUM,
