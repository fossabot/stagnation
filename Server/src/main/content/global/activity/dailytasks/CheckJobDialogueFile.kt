package content.global.activity.dailytasks

import config.Items
import content.global.activity.dailytasks.impl.BoneBuryingJobs
import content.global.activity.dailytasks.impl.CombatJobs
import content.global.activity.dailytasks.impl.ProductionJobs
    ...
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        when (stage) {
            START_DIALOGUE -> showTopics(
                IfTopic(FacialExpression.HAPPY, "I have the items you wanted.", TURN_IN_ITEMS_1, (job.type == JobType.PRODUCTION && amountInInventory(player!!, (job as ProductionJobs).itemId) > 0)),
            ...
            GET_NEW_JOB_4 -> npcl("There, I've updated your task list to show your new job. Best of luck!").also { stage = GET_NEW_JOB_5 }
            GET_NEW_JOB_5 -> playerl("Thanks.").also { stage = END_DIALOGUE }
            GET_NEW_JOB_NONE -> npcl("I'm sorry, I don't currently have any jobs that you're qualified for.").also { stage = END_DIALOGUE }
            GET_NEW_JOB_REACHED_LIMIT -> npcl("You've hit your limit for the day. Come back tomorrow!").also { stage = END_DIALOGUE }
        }
    }
}