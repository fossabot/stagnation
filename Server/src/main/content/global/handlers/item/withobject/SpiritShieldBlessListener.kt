    ...
    override fun defineListeners() {
        onUseWith(IntType.SCENERY, COMPONENTS, *ALTARS) { player, _, _ ->
            ...
            if (!hasLevelDyn(player, Skills.PRAYER, REQUIRED_PRAYER_LEVEL_TO_BLESS_SHIELD)) {
                sendMessage(
                    player,
                    "You need a Prayer level of $REQUIRED_PRAYER_LEVEL_TO_BLESS_SHIELD in order to bless the shield."
                )
            }
        ...
        onUseWith(IntType.SCENERY, SIGILS_SHIELDS_MAP.keys.toIntArray(), *ANVILS) { player, used, _ ->
            ...
            if (!hasLevelDyn(player, Skills.PRAYER, REQUIRED_PRAYER_LEVEL_TO_ATTACH_SIGIL) || !hasLevelDyn(
                    player,
                    Skills.SMITHING,
                    REQUIRED_SMITHING_LEVEL_TO_ATTACH_SIGIL
                )
            ) {
                sendMessage(
                    player,
                    "You need a Prayer level of $REQUIRED_PRAYER_LEVEL_TO_ATTACH_SIGIL and a Smithing level of $REQUIRED_SMITHING_LEVEL_TO_ATTACH_SIGIL in order to attach the sigil."
                )
                return@onUseWith true
            }
                    ...
                    2 -> {
                        if (removeItem(player, used) && removeItem(player, Items.BLESSED_SPIRIT_SHIELD_13736)) {
                            ...
                            sendItemDialogue(
                                player,
                                product.id,
                                "You successfully attach the " + used.name.lowercase(Locale.getDefault()) + " to the blessed spirit shield."
                            )
                        }
                    }
                }
