...
/**
 * Represents interaction listener for Ammo mould.
 */
class AmmoMouldOnFurnace : InteractionListener {
    private val furnaces = intArrayOf(
        ...
        36956,
        37651
    )  // abstract when smelting converted to kotlin
    ...
    private fun cannonBallOnUseWithHandler(player: Player, used: Node, with: Node): Boolean {
        ...
        if (!isQuestComplete(player, content.quest.member.dwarfcannon.DwarfCannon.NAME)) {
            sendDialogue(
                player,
                "You need to complete the ${content.quest.member.dwarfcannon.DwarfCannon.NAME} quest in order to do this."
            )
            return true
        }
