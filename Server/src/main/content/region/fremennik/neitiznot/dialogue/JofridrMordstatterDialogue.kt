...
/**
 * Represents the Jofridr Mordstatter dialogue plugin.
 */
@Initializable
class JofridrMordstatterDialogue(player: Player? = null) : DialoguePlugin(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            1 -> when (buttonId) {
                1 -> playerl(FacialExpression.NEUTRAL, "Yes please, Jofridr.").also { stage = END_DIALOGUE }.also { openNpcShop(player, NPCs.JOFRIDR_MORDSTATTER_5509) }
