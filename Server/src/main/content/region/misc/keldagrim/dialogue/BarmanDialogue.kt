...
/**
 * Represents the dialogue plugin used for the Barman,
 * King's Axe Inn.
 */
@Initializable
class BarmanDialogue(player: Player? = null) : DialoguePlugin(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            17 -> when (buttonId) {
                1 -> if (amountInInventory(player, Items.COINS_995) <= 2) {
                    player(FacialExpression.NEUTRAL, "I'm sorry, I seem to have run out of money.")
                    stage = 100
                } else {
