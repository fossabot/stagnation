...
import config.NPCs
import content.quest.member.deathplateau.dialogue.DunstanDPDialogue
...
/**
 * Represents the Dunstan dialogue plugin.
 */
@Initializable
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        ...
        if (isQuestComplete(player!!, "Troll Stronghold")) {
            when (stage) {
                ...
                17 -> npcl(FacialExpression.FRIENDLY, "Anything else before I get on with my work?").also { stage++ }
            }
            return true
        }
        ...
        if (isQuestInProgress(player!!, "Troll Stronghold", 1, 99)) {
            openDialogue(player!!, DunstanDPDialogue(), npc)
            return true
        }
