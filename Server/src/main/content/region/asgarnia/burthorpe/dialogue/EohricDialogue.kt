...
import config.NPCs
import content.quest.member.deathplateau.dialogue.EohricDPDialogue
...
/**
 * Represents the Eohric dialogue plugin.
 */
@Initializable
class EohricDialogue(player: Player? = null) : DialoguePlugin(player) {
    override fun open(vararg args: Any?): Boolean {
        ...
        if (!isQuestComplete(player,"Death Plateau")) {
            openDialogue(player!!, EohricDPDialogue(), npc)
        } else {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (stage) {
            ...
            9 -> npcl(FacialExpression.FRIENDLY, "But rest assured that he is working tirelessly to maintain the safety and wellbeing of Burthorpe's people.").also { stage = 2 }
        }
        return true
    }
