...
/**
 * Represents the plugin used for buying a battlestaffs from Zeke.
 */
@Initializable
    ...
    class ZaffDialogue : DialoguePlugin {
        ...
        override fun handle(interfaceId: Int, buttonId: Int): Boolean {
            when (stage) {
                ...
                22 -> end()
                50 -> {
                    if (inInventory(player, 11014, 1)) interpreter.sendDialogues(npc, FacialExpression.HALF_GUILTY, "Go and get the one that's in your inventory " + player.username + "!")
                    else if (player.bank.contains(11014, 1)) interpreter.sendDialogues(npc, FacialExpression.HALF_GUILTY, "Go and get the one that's in your bank" + player.username + "!")
                    else if (inEquipment(player, 11014, 1)) interpreter.sendDialogues(npc, FacialExpression.HALF_GUILTY, "Go and get the one that's on your finger " + player.username + "!")
                    else {
                        interpreter.sendDialogues(npc, FacialExpression.HALF_GUILTY, "Of course you can! Here you go " + player.username + "!")
        ...
        override fun handle(interfaceId: Int, buttonId: Int): Boolean {
            when (stage) {
                0 -> {
                    ...
                    interpreter.sendDialogues(546, FacialExpression.HALF_GUILTY, "Battlestaves cost 7,000 gold pieces each. I have ${maxStaffs - ammount} left.", "How many would you like to buy?")
                    stage = 1
                }
