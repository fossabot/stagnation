...
/**
 * Handles Player safety interactions.
 */
public class PSOptionHandler extends OptionHandler {

	@Override
	public Plugin<Object> newInstance(Object arg) throws Throwable {
		SceneryDefinition.forId(StrongHoldOfPlayerSafetyPlugin.JAIL_ENTRANCE_ID_ENTER).getHandlers().put("option:use", this);
		SceneryDefinition.forId(StrongHoldOfPlayerSafetyPlugin.JAIL_ENTRANCE_LEAVE).getHandlers().put("option:leave", this);
		SceneryDefinition.forId(StrongHoldOfPlayerSafetyPlugin.JAIL_STAIRS_UP).getHandlers().put("option:climb-up", this);
		SceneryDefinition.forId(StrongHoldOfPlayerSafetyPlugin.JAIL_STAIRS_DOWN).getHandlers().put("option:climb-down", this);
		...
		ItemDefinition.forId(StrongHoldOfPlayerSafetyPlugin.TEST_PAPER_ITEM_ID).getHandlers().put("option:take exam", this);
		SceneryDefinition.forId(29729).getHandlers().put("option:climb", this);
		for (StrongHoldOfPlayerSafetyPlugin.JailPlaques plaque : StrongHoldOfPlayerSafetyPlugin.JailPlaques.values()) {
	...
	public boolean handle(Player player, Node node, String option) {
		...
		if (node instanceof Scenery) {
			...
			StrongHoldOfPlayerSafetyPlugin.JailPlaques plaque = StrongHoldOfPlayerSafetyPlugin.forId(object.getId());
			if (plaque != null) {
				plaque.read(player, object);
				return true;
			} else if (object.getId() == StrongHoldOfPlayerSafetyPlugin.JAIL_ENTRANCE_ID_ENTER || object.getId() == StrongHoldOfPlayerSafetyPlugin.JAIL_ENTRANCE_LEAVE) {
				player.getProperties().setTeleportLocation(object.getId() == StrongHoldOfPlayerSafetyPlugin.JAIL_ENTRANCE_ID_ENTER ? StrongHoldOfPlayerSafetyPlugin.JAIL_ENTRANCE_LOCATION_ENTER : StrongHoldOfPlayerSafetyPlugin.JAIL_ENTRANCE_LOCATION_LEAVE);
				return true;
			} else if (object.getId() == StrongHoldOfPlayerSafetyPlugin.JAIL_STAIRS_UP || object.getId() == StrongHoldOfPlayerSafetyPlugin.JAIL_STAIRS_DOWN) {
				if (!player.getSavedData().getGlobalData().hasReadPlaques() && object.getId() != StrongHoldOfPlayerSafetyPlugin.JAIL_STAIRS_DOWN) {
					player.getPacketDispatch().sendMessage("You need to read the jail plaques before the guard will allow you upstairs.");
					return true;
				}
				player.getProperties().setTeleportLocation(object.getId() == StrongHoldOfPlayerSafetyPlugin.JAIL_STAIRS_UP ? StrongHoldOfPlayerSafetyPlugin.JAIL_UP_STAIRS : StrongHoldOfPlayerSafetyPlugin.JAIL_DOWN_STAIRS);
				return true;
			}
			...
			if (item.getId() == StrongHoldOfPlayerSafetyPlugin.TEST_PAPER_ITEM_ID) {
				removeAttribute(player, "s-stage");
