    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when(stage){
            ...
            2 -> when(buttonId){
                ...
                2 -> player(FacialExpression.NEUTRAL, "No thank you, I'm fine.").also { stage = 99 }
                3 -> player(FacialExpression.ASKING, "What's it like living down here?").also { stage = 20 }
            }
            10 -> end().also { openNpcShop(player, NPCs.HALLA_3921) }
            ...
            21 -> npc(FacialExpression.NEUTRAL, "The only problem I find is that the lighting's not very good,", "which means I make mistakes when cutting cloth.").also { stage = 99 }
            99 -> end()
        }
        return true
    }
