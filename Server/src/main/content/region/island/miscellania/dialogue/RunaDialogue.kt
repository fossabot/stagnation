...
/**
 * Represents the Runa dialogue plugin.
 */
@Initializable
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when(stage){
            ...
            3 -> when(buttonId){
                ...
                2 -> player(FacialExpression.NEUTRAL, "No thank you.").also { stage = 99 }
                3 -> player(FacialExpression.ASKING, "What's it like living down here?").also { stage = 20 }
            }
            10 -> end().also { openNpcShop(player, NPCs.RUNA_3920) }
            20 -> npc(FacialExpression.HALF_WORRIED, "Business is booming!").also { stage++ }
            21 -> npc(FacialExpression.HALF_WORRIED, "Now, if only I hadn't taken a loss to the beer I sold", "to those teenagers.").also { end() }
            99 -> end()
        }
        return true
    }
