...
/**
 * Represents the Osvald dialogue plugin.
 */
@Initializable
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when(stage){
            ...
            2 -> when(buttonId){
                ...
                2 -> player(FacialExpression.NEUTRAL, "No thank you, I don't need food just now.").also { end() }
                3 -> player(FacialExpression.ASKING, "What's it like living down here?").also { stage = 20 }
            }
            10 -> {
                end().also { openNpcShop(player, NPCs.OSVALD_3923) }
            }
            20 -> npc(FacialExpression.FRIENDLY, "The town's thriving.", "I'm sure it'll soon be as busy as Rellekka!").also { stage = 99 }
            99 -> end()
        }
        return true
    }
