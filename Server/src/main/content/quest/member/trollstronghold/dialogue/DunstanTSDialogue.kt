...
/**
 * Represents the Dunstan dialogue file used in Troll Stronghold quest.
 */
class DunstanTSDialogue : DialogueFile() {
    override fun handle(componentID: Int, buttonID: Int) {

        when {
            (getQuestStage(player!!, "Troll Stronghold") in 1..10) -> {
                when (stage) {
                    0 -> npcl(FacialExpression.FRIENDLY, "Have you managed to rescue Godric yet?").also { stage++ }
            ...
            (getQuestStage(player!!, "Troll Stronghold") == 11) -> {
                when (stage) {
                    0 -> playerl(FacialExpression.FRIENDLY, "Has Godric returned home?").also { stage++ }
                    ...
                    4 -> finishQuest(player!!, "Troll Stronghold").also { stage = END_DIALOGUE }
