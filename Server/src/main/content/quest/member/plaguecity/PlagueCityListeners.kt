...
import config.Sounds.PLAGUE_ATTACH_1731
import content.quest.member.plaguecity.dialogue.HeadMournerPCDialogue
import content.quest.member.plaguecity.dialogue.ManRehnisonPCDialogue
import content.quest.member.plaguecity.dialogue.MournerPCDialogue
import content.quest.member.plaguecity.dialogue.WomanPCDialogue
import core.api.*
...
class PlagueCityListeners : InteractionListener {
    ...
    override fun defineListeners() {
        ...
        on(HEAD_MOURNER, IntType.NPC, "talk-to") { player, _ ->
            openDialogue(player, HeadMournerPCDialogue())
            return@on true
        }

        on(MANS, IntType.NPC, "talk-to") { player, _ ->
            if(inBorders(player, 2496, 3280,2557, 3336)) {
                openDialogue(player, ManRehnisonPCDialogue())
            }
            return@on true
        }
        ...
        on(WOMANS, IntType.NPC, "talk-to") { player, _ ->
            if(inBorders(player, 2496, 3280,2557, 3336)) {
                openDialogue(player, WomanPCDialogue())
            }
            return@on true
        }

        on(LEFT_DOOR, IntType.SCENERY, "open") { player, _ ->
        ...
        on(RIGHT_DOOR, IntType.SCENERY, "open") { player, _ ->
            if(inBorders(player, 2556, 3298, 2557, 3301)){
                lock(player,2)
                sendMessage(player, "You pull on the large wooden doors...")
                runTask(player,2){
        ...
        on(HEAD_DOORS, IntType.SCENERY, "open") { player, node ->
            if (player.questRepository.getStage("Plague City") == 11) {
                ...
                openDialogue(player, MournerPCDialogue())
            } else if (player.questRepository.getStage("Plague City") > 16) {
                DoorActionHandler.handleAutowalkDoor(player, node.asScenery())
            } else {
                openDialogue(player, MournerPCDialogue())
                    ...
                    override fun pulse(): Boolean {
                        when (counter++) {
                            ...
                            3 -> {
                                ...
                                playAudio(player, (PLAGUE_ATTACH_1731))
