package content.quest.member.thelosttribe
...
class LostTribe : Quest("Lost Tribe",84,83,1) {
    ...
    override fun getConfig(player: Player?, stage: Int): IntArray {
        player ?: return intArrayOf(Vars.VARP_QUEST_LOST_TRIBE_465,0)
        if(stage in 50..99){
            return intArrayOf(Vars.VARP_QUEST_LOST_TRIBE_465,9)
        }
        if(stage in 46..49){
            return intArrayOf(Vars.VARP_QUEST_LOST_TRIBE_465,8)
        }
        if(stage in 44..45){
            return intArrayOf(Vars.VARP_QUEST_LOST_TRIBE_465,7)
        }
        if(stage in 30..43 && getAttribute(player,"tlt-hole-cleared",false)){
            return intArrayOf(Vars.VARP_QUEST_LOST_TRIBE_465,4)
        }
        if(stage >= 100) return intArrayOf(Vars.VARP_QUEST_LOST_TRIBE_465,11)
        if(stage > 0) return intArrayOf(Vars.VARP_QUEST_LOST_TRIBE_465,1)
        return intArrayOf(Vars.VARP_QUEST_LOST_TRIBE_465,0)
    }
}
