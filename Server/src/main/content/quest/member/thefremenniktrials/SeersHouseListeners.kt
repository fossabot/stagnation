package content.quest.member.thefremenniktrials
...
/**
 * Handles interaction in Seer house for Fremennik trials quest.
 */
class SeersHouseListeners : InteractionListener {
    ...
    override fun defineListeners() {
        ...
        on(CHEST, IntType.SCENERY, "Open"){ _, node ->
            SceneryBuilder.replace(node.asScenery(), node.asScenery().transform(4168))
            return@on true
        }

        on(CHEST, IntType.SCENERY, "Close"){ _, node ->
        ...
        on(BULLSHEAD, IntType.SCENERY, "Study") { player, _ ->
            player.dialogueInterpreter.open(BullHeadDialogue(player))
            return@on true
        }

        on(UNICORNHEAD, IntType.SCENERY, "Study") { player, _ ->
