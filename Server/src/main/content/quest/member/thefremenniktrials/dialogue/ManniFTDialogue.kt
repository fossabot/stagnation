...
/**
 * Represents the Manni dialogue for Fremennik trials quest.
 */
@Initializable
class ManniFTDialogue(player: Player? = null) : DialoguePlugin(player){
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when(stage){
            ...
            9 -> when(buttonId){
                ...
                11 -> {npc("We start when you have your keg of beer with you","and finish when one of us can drink no more and","yields.");stage = 1000;setAttribute(player,"/save:fremtrials:manni-accepted",true)}
            ...
            104 -> {
                GameWorld.submit(DrinkingPulse(player,curNPC, getAttribute(player,"fremtrials:keg-mixed",false))); end()}
            ...
            190 -> npcl(FacialExpression.HAPPY,"Hey! It's ${getAttribute(player,"fremennikname","clyde smilers")}! Let me buy you a drink!").also {
        ...
        override fun pulse(): Boolean {
            if(!lowAlcohol!!){
                when(counter++){
                    ...
                    5 -> {player?.animator?.animate(Animation(1330,Animator.Priority.HIGH)); player?.inventory?.remove(Item(Items.KEG_OF_BEER_3711))}
                ...
                when(counter++){
                    ...
                    22 -> {player?.unlock(); npc?.unlock(); player?.face(player);npc?.face(npc); npc?.isNeverWalks = false;removeAttribute(player!!,"fremtrials:cherrybomb");removeAttribute(player,"fremtrials:manni-accepted");removeAttribute(player,"fremtrials:keg-mixed"); setAttribute(player,"/save:fremtrials:manni-vote",true); setAttribute(player,"/save:fremtrials:votes",getAttribute(player,"fremtrials:votes",0) + 1); return true}
                }
            }
            return false
        }
