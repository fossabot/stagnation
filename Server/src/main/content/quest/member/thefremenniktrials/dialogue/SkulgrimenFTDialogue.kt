...
/**
 * Represents the Skulgrimen dialogue for Fremennik trials quest.
 */
@Initializable
class SkulgrimenFTDialogue(player: Player? = null) : DialoguePlugin(player) {
    ...
    override fun open(vararg args: Any?): Boolean {
        ...
        if(getAttribute(player,"sigmund-steps", 0) == 7){
            ...
            npcl(FacialExpression.HAPPY,"Hello again, ${getAttribute(player,"fremennikname","ringo")}. Come to see what's for sale?")
            stage = 1001
            return true
        }
        else{
    ...
    override fun newInstance(player: Player?): DialoguePlugin {
        return SkulgrimenFTDialogue(player)
    }
