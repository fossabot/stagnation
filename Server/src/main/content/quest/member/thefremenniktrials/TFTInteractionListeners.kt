...
import content.quest.member.thefremenniktrials.npc.KoscheiSession
import core.api.*
...
/**
 * Handles main interactions for Fremennik trials quest.
 */
        ...
        on(LYRE_IDs, IntType.ITEM, "play"){ player, lyre ->
            if(getAttribute(player,"onStage",false) && !getAttribute(player,"lyreConcertPlayed",false)){
                ...
                if(removeItem(player,lyre.asItem())) {
                    ...
                    Pulser.submit(LyreTeleport.LyreTeleportPulse(player))
                }
            }
            return@on true
        }
