package content.quest.member.thegrandtree

import config.Items
import config.NPCs
import config.Sounds
import content.quest.member.thegrandtree.cutscene.BlackDemonCutscene
import content.quest.member.thegrandtree.dialogue.KingNarnodeGTDialogue
import content.quest.member.thegrandtree.dialogue.KingNarnodeUnderGroundDialogue
import content.quest.member.thegrandtree.dialogue.ShipyardWorkerGTDialogueFile
import core.api.*
...
/**
 * Main interactions for Grand tree quest.
 */
class GrandTreeListeners: InteractionListener {
    ...
    val karamjaGate = intArrayOf(2438,2439)
    val hazelmerescroll = Items.HAZELMERES_SCROLL_786
    val lumberorder = Items.LUMBER_ORDER_787
    val invasionplans = Items.INVASION_PLANS_794
    val allScrolls = intArrayOf(hazelmerescroll,lumberorder,invasionplans)
    ...
    val invasionPlans = arrayOf(
        ...
        "",
        "Fleet 3",
        "Sails north to attack Kandarin from south, reinforced",
        "by gnome foot soldiers leaving gnome stronghold.",
        "",
        "${RED}Take no prisoners!",
    )
    ...
    override fun defineListeners() {
        on(NPCs.KING_NARNODE_SHAREEN_670, IntType.NPC, "talk-to"){ player, npc ->
            ...
            if(player.location.regionId == aboveground)
                openDialogue(player, KingNarnodeGTDialogue(), npc)
            else
                openDialogue(player, KingNarnodeUnderGroundDialogue(), npc)
            return@on true
        }

        on(allScrolls, IntType.ITEM, "read") { player, node ->
            scrollHandler(player, node.asItem())
            return@on true
        }

        on(2444, IntType.SCENERY, "open"){ player, node ->
            if(node.location == Location(2487,3464,2) && !isQuestComplete(player, TheGrandTree.questName)){
                if(getAttribute(player, "/save:grandtree:twig1", false) &&
                    getAttribute(player, "/save:grandtree:twig2", false) &&
                    getAttribute(player, "/save:grandtree:twig3", false) &&
                    getAttribute(player, "/save:grandtree:twig4", false)){
        ...
        on(2446, IntType.SCENERY, "open"){ player, node ->
            if(node.location == Location(2463, 3497, 0) && isQuestComplete(player!!, TheGrandTree.questName)){
                player.animator.animate(Animation(828))
                // Go to tunnels
        ...
        onUseWith(IntType.SCENERY, 788, 2436){ player, used, with ->
            ...
            if(getQuestStage(player, TheGrandTree.questName) < 60)
        ...
        on(2435, IntType.SCENERY, "search") { player, _ ->
            if (getQuestStage(player, TheGrandTree.questName) == 47 && !inInventory(player, Items.GLOUGHS_JOURNAL_785)) {
                ...
                sendMessage(player, "You have no free space to hold any more items.")
            } else {
                sendMessage(player, "The cupboard is empty.")
            }
            return@on true
        }
        ...
        on(32319, IntType.SCENERY, "search"){ player, node ->
            if(getQuestStage(player, TheGrandTree.questName) < 99 || player.hasItem(Item(Items.DACONIA_ROCK_793))){ return@on true; }
        ...
        on(karamjaGate, IntType.SCENERY, "open"){ player, _ ->
            openDialogue(player, ShipyardWorkerGTDialogueFile(), NPC(NPCs.SHIPYARD_WORKER_675))
            return@on true
        }
    ...
    private fun scrollHandler(player: Player, item: Item) {
        ...
        openInterface(player, 222).also {
            when (id) {
                hazelmerescroll -> setInterfaceText(player, hazelmerescrollText.joinToString("<br>"), 222, 5)
                lumberorder -> setInterfaceText(player, lumberOrder.joinToString("<br>"), 222, 1)
                invasionplans -> setInterfaceText(player, invasionPlans.joinToString("<br>"), 222, 1)
            }
        }
    }
}
