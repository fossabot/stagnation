...
/**
 * Represents Glough dialogue in the grand tree quest.
 */
        ...
        when (getQuestStage(player!!, TheGrandTree.questName)) {
            40 -> {
                when (stage) {
                    ...
                    9 -> npcl(FacialExpression.OLD_DEFAULT,"Your type can't be trusted! I'll take care of this! Go back to the King.").also {
                        setQuestStage(player!!, TheGrandTree.questName, 45)
                        stage = END_DIALOGUE
                    }
                }
            }
                        ...
                        GameWorld.Pulser.submit(object : Pulse(0) {
                            ...
                            override fun pulse(): Boolean {
                                when (count) {
                                    ...
                                    8 -> {
                                        npc.clear()
                                        setQuestStage(player!!, TheGrandTree.questName, 50)
            ...
            55 -> {
                when(stage){
                    ...
                    2 -> playerl(FacialExpression.FRIENDLY,"You may be able to make a fleet but the tree gnomes will never follow you into battle against humans.").also { stage++ }
                    3 -> npcl(FacialExpression.OLD_DEFAULT,"So, you know more than I thought! The gnomes fear humanity more than any other race. I just need to give them a push in the right direction. There's nothing you can do traveller! Leave before it's too late!").also { stage++ }
                    4 -> playerl(FacialExpression.FRIENDLY,"King Narnode won't allow it!").also { stage++ }
                    5 -> npcl(FacialExpression.OLD_DEFAULT,"The King's a fool and a coward! He'll bow to me! You'll soon be back in that cage!").also { stage = END_DIALOGUE }
                }
            }
            60 -> {
                when(stage){
                    0 -> playerl(FacialExpression.FRIENDLY,"I'm going to stop you, Glough!").also { stage++ }
                    1 -> npcl(FacialExpression.OLD_DEFAULT,"You're becoming quite annoying traveller!").also { stage++ }
                    ...
                    4 -> npcl(FacialExpression.OLD_DEFAULT,"Leave human, before I have you put in the cage!").also { stage = END_DIALOGUE }
                }
            }
            else -> {
                when (stage) {
                    ...
                    3 -> npcl(FacialExpression.OLD_DEFAULT,"The Gnome Stronghold is for gnomes alone!").also { stage++ }
                    4 -> playerl(FacialExpression.FRIENDLY,"Surely not!").also { stage++ }
                    5 -> npcl(FacialExpression.OLD_DEFAULT,"We don't need your sort round here!").also { stage = END_DIALOGUE }
                }
            }
        }
    }
