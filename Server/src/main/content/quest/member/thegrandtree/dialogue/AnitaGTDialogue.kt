...
/**
 * Represents Anita dialogue in the grand tree quest.
 */
@Initializable
class AnitaGTDialogue(player: Player? = null) : DialoguePlugin(player){
            ...
            60 -> {
                if(player!!.hasItem(Item(Items.GLOUGHS_KEY_788)) && stage < 12){
                    ...
                    when(stage){
                        ...
                        7 -> when(buttonId){
                            1 -> playerl(FacialExpression.THINKING,"I suppose so.").also { stage++ }
                            2 -> playerl(FacialExpression.NEUTRAL,"No, I'm busy.").also { stage = END_DIALOGUE }
                        }
                        8 -> npcl(FacialExpression.OLD_DEFAULT,"Please give this key to Glough, he left it here last night.").also { stage++ }
                        9 -> sendDialogue(player!!, "Anita gives you a key.").also {
                            addItemOrDrop(player!!, Items.GLOUGHS_KEY_788)
                            stage++
                        }
                        10 -> npcl(FacialExpression.OLD_DEFAULT,"Thanks a lot.").also { stage++ }
                        11 -> playerl(FacialExpression.HAPPY,"No... thank you!").also { stage = END_DIALOGUE }
                    }
                }
            }
        }
