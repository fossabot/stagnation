package content.quest.member.bigchompybirdhunting

import config.Animations
import config.Graphics
import config.Items
import config.NPCs
import config.Vars
import core.api.*
...
@Initializable
class BigChompyBirdHunting : Quest("Big Chompy Bird Hunting", 35, 34, 2, Vars.VARP_QUEST_CHOMPY_293, 0, 1, 65), InteractionListener {
  companion object {
      ...
      submitIndividualPulse(player, object : Pulse(3) {
        override fun pulse(): Boolean {
          ...
          if (removeItem(player, Item(Items.OGRE_ARROW_SHAFT_2864, iterAmount)) && removeItem(
              player,
              Item(Items.FEATHER_314, iterAmount)
            )
          ) {
