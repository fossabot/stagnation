package content.quest.member.betweenarock
...
class BetweenARock : Quest("Between a Rock...", 34, 33, 2,Vars.VARBIT_QUEST_BETWEEN_A_ROCK_299,0,1,110) {
    ...
    companion object {
        const val BetweenARock = "Between a Rock..."
    }
    ...
    override fun drawJournal(player: Player?, stage: Int) {
        ...
        if (stage == 0) {
            ...
            line(player, if (hasLevelStat(player, Skills.DEFENCE, 30)) "---Level 30 defence/--" else "!!30 defence??", line++)
            line(player, if (hasLevelStat(player, Skills.MINING, 40)) "---Level 40 mining (higher mining being an advantage)/--" else "!!Level 40 mining (higher mining being an advantage)??", line++)
            line(player, if (hasLevelStat(player, Skills.SMITHING, 50)) "---Level 50 smithing/--" else "!!Level 50 smithing??", line++)
        ...
        if (stage == 1) {
            ...
            line(player, "I have found the missing pages of a book. Rolad has given me the assembled version.", line++, false)
            ...
            line(player, "how a demonic spirit was imprisoned inside the rock through 'yellow power stones'.", line++, false)
            ...
            line(player, "I have been shot into the rock and have spoken to the spirit that lives inside!", line++, false)
