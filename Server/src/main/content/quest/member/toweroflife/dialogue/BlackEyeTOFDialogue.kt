...
/**
 * Represents the Black Eye dialogue plugin for Tower of Life quest.
 */
@Initializable
class BlackEyeTOFDialogue(player: Player? = null) : DialoguePlugin(player) {
    ...
    override fun handle(componentID: Int, buttonID: Int): Boolean {
        when (getQuestStage(player!!, "Tower Of Life")) {
            0 -> when (stage) {
                ...
                8 -> playerl(FacialExpression.FRIENDLY, "Erm...yeah...no...sorry...gotta go.").also { stage = END_DIALOGUE }
            }
            2 -> when(stage){
                ...
                2 -> playerl(FacialExpression.FRIENDLY, "I was just wondering if you might be able to help me out with one?").also { stage++ }
                3 -> npcl(FacialExpression.FRIENDLY, "Always glad to help a budding builder.").also { stage++ }
                4 -> npcl(FacialExpression.FRIENDLY, "But first, prove your Construction knowledge by answering some questions.").also { stage++ }
                5 -> npcl(FacialExpression.FRIENDLY, "How many nails does it take to make a rocking chair?").also { stage++ }
                ...
                8 -> npcl(FacialExpression.FRIENDLY, "One? How do you suppose to use a single nail?").also { stage = END_DIALOGUE }
                9 -> npcl(FacialExpression.FRIENDLY, "Nope. Keep guessing!").also { stage = END_DIALOGUE }
                10 -> npcl(FacialExpression.FRIENDLY, "Bingo! Okay, now what takes 3 planks, 3 cloths, and 3 nails to make, and help remove light from a room?").also { stage++ }
                ...
                13 -> npcl(FacialExpression.FRIENDLY, "Of course, because that's what everyone needs - a rug up at their window.").also { stage = END_DIALOGUE }
                14 -> npcl(FacialExpression.FRIENDLY, "Those curtains are not going to be in any way opulent, matey.").also { stage = END_DIALOGUE }
                15 -> npcl(FacialExpression.FRIENDLY, "Nice one. Last question: I like fish and I want to put some in my garden, but I need a special water feature. What materials would I require?").also { stage++ }
                ...
                21 -> npcl(FacialExpression.FRIENDLY, "You seem to know your stuff. I got a spare helmet from a builder that died on... I mean, err, had to leave the job for greener pastures."). also { stage++ }
                ...
                24 -> playerl(FacialExpression.FRIENDLY, "Wait a minute! Is there blood on this helmet?").also { stage++ }
                25 -> npcl(FacialExpression.FRIENDLY, "Erm, no, that's just, erm, paint. Yes, paint! That's what it is!").also { stage++ }
