 ...
 */
@Initializable
class EnlightenedJourney : Quest("Enlightened Journey", 55, 54, 1, Vars.VARBIT_QUEST_ENLIGHTENED_JOURNEY_2866, 0, 1, 200) {
    ...
    override fun drawJournal(player: Player?, stage: Int) {
        ...
        if (stage >= 0) {
            ...
            line(player, if (hasLevelStat(player, Skills.CRAFTING, 36)) "---Level 36 Crafting/--" else "!!Level 36 Crafting??", line++)
            line(player, if (hasLevelStat(player, Skills.FARMING, 30)) "---Level 30 Farming/--" else "!!Level 30 Farming??", line++)
            line(player, if (hasLevelStat(player, Skills.FIREMAKING, 20)) "---Level 20 Firemaking/--" else "!!Level 20 Firemaking??", line++)
    ...
    override fun finish(player: Player?) {
        ...
        setVarbit(player, Vars.VARBIT_QUEST_ENLIGHTENED_JOURNEY_2866, 200, true)
        setVarbit(player, Vars.VARBIT_BALLOON_TRANSPORT_SYSTEM_ENTRANA_2867, 2, true)
        setVarbit(player, Vars.VARBIT_BALLOON_TRANSPORT_SYSTEM_TAVERLY_2868, 1, true)
        setVarbit(player, Vars.VARBIT_BALLOON_TRANSPORT_SYSTEM_CASTLE_WARS_2869, 1, true)
        setVarbit(player, Vars.VARBIT_BALLOON_TRANSPORT_SYSTEM_GRAND_TREE_2870, 1, true)
        setVarbit(player, Vars.VARBIT_BALLOON_TRANSPORT_SYSTEM_CRAFTING_GUILD_2871, 1, true)
        setVarbit(player, Vars.VARBIT_BALLOON_TRANSPORT_SYSTEM_VARROCK_2872, 1, true)
