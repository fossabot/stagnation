...
import content.quest.member.deathplateau.dialogue.DoorDPDialogue
import content.quest.member.deathplateau.dialogue.IOUNoteDialogue
import core.api.*
...
/**
 * Listeners for Death Plateau quest.
 */
    ...
    override fun defineListeners() {
        on(Scenery.DOOR_3747, SCENERY, "open") { player, _ ->
            ...
            if (player.location == location(2906, 3543, 1)) {
                openDialogue(player, DoorDPDialogue(1))
            } else {
        ...
        on(Scenery.DOOR_3745, SCENERY, "open") { player, node ->
            if (node.location == location(2823, 3555, 0)) {
                // 1st Door to Tenzing
                openDialogue(player, DoorDPDialogue(2))
            } else if (node.location == location(2820, 3558, 0)) {
                // 2nd Door to chicken pen
                openDialogue(player, DoorDPDialogue(3))
            }
            return@on true
        }

        on(Items.IOU_3103, ITEM, "read") { player, _ ->
            openDialogue(player, IOUNoteDialogue())
            return@on true
        }
