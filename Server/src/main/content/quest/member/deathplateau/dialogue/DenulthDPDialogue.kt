package content.quest.member.deathplateau.dialogue
...
/**
 * Represents the Denulth plugin for Death Plateau quest.
 */
@Initializable
class DenulthDPDialogue : DialogueFile() {
    override fun handle(componentID: Int, buttonID: Int) {
        ...
        when (getQuestStage(player!!, "Death Plateau")){
            ...
            in 5..21 -> when(stage){
                ...
                2 -> when(buttonID){
                    1 -> playerl(FacialExpression.HALF_ASKING, "Can you remind me of the quest I am on?").also { stage = 5 }
            ...
            22 -> when(stage){
                ...
                10 -> sendItemDialogue(player!!, Items.CERTIFICATE_3114, "Denulth has given you a certificate.").also { stage++ }
            ...
            in 24..26 -> when(stage){
                0 -> playerl(FacialExpression.FRIENDLY, "Hello!").also { stage++ }
                1 -> npcl(FacialExpression.FRIENDLY, "Hello citizen, have you found the secret way up Death Plateau?").also { stage++ }
                2 -> if (removeItem(player!!, Items.SECRET_WAY_MAP_3104)) {
                ...
                3 -> sendItemDialogue(player!!, Items.SECRET_WAY_MAP_3104, "You give Denulth the map of the secret way.").also { stage++ }.also { sendMessage(player!!, "You give a Denulth the map of the secret way.") }
                ...
                6 -> if(removeItem(player!!, Items.COMBINATION_3102)){
                ...
                7 -> sendItemDialogue(player!!, Items.COMBINATION_3102, "You give Denulth the combination to the equipment room.").also { stage++ }.also { sendMessage(player!!, "You give Denulth the combination to the equipment room.") }
            ...
            100 -> when(stage){
                0 -> if(getQuestStage(player!!, "Troll Stronghold") >= 1) {
                ...
                2 -> options("How goes your fight with the trolls?", "I thought the White Knights controlled Asgarnia?", "See you about Denulth!").also { stage++ }
                3 -> when(buttonID){
                    1 -> playerl(FacialExpression.HALF_ASKING, "How goes your fight with the trolls?").also { stage++ }
                    2 -> playerl(FacialExpression.NEUTRAL, "I thought the White Knights controlled Asgarnia?").also { stage = 13 }
                    3 -> playerl(FacialExpression.ASKING, "See you about Denulth!").also { stage = 17 }
                }
                4 -> npcl(FacialExpression.HALF_WORRIED, "I'm afraid I have bad news. We made our attack as planned, but we met unexpected resistance.").also { stage++ }
                5 -> playerl(FacialExpression.HALF_ASKING, "What happened?").also { stage++ }
                6 -> npcl(FacialExpression.WORRIED, "We were ambushed by trolls coming from the north. They captured Dunstan's son, Godric, who we enlisted at your request, we tried to follow but we were repelled at the foot of their stronghold.").also { stage++ }
                7 -> options("I'm sorry to hear that.", "Is there anything I can do to help?").also { stage++ }
                8 -> when(buttonID){
                    1 -> playerl(FacialExpression.NEUTRAL, "I'm sorry to hear that.").also { stage = END_DIALOGUE }
                    2 -> playerl(FacialExpression.ASKING, "Is there anything I can do to help?").also { stage++ }
                }
                9 -> npcl(FacialExpression.HALF_WORRIED, "The way to the stronghold is treacherous, friend. Even if you manage to climb your way up, there will be many trolls defending the stronghold.").also { stage++ }
                10 -> playerl(FacialExpression.FRIENDLY, "I'll get Godric back!").also { stage++ }
                11 -> npcl(FacialExpression.HAPPY, "God speed friend! I would send some of my men with you, but none of them are brave enough to follow.").also { stage++ }
                12 -> {
                    end()
                    setQuestStage(player!!, "Troll Stronghold", 1)
                }
                13 -> npcl(FacialExpression.ANGRY, "You are right citizen. The White Knights have taken advantage of the old and weak king, they control most of Asgarnia, including Falador. However they do not control Burthorpe!").also { stage++ }
                14 -> npcl(FacialExpression.EVIL_LAUGH, "We are the prince's elite troops! We keep Burthorpe secure!").also { stage++ }
                15 -> npcl(FacialExpression.ANGRY, "The White Knights have overlooked us until now! They are pouring money into their war against the Black Knights, They are looking for an excuse to stop our funding and I'm afraid they may have found it!").also { stage++ }
                16 -> npcl(FacialExpression.HALF_WORRIED, "If we can not destroy the troll camp on Death Plateau then the Imperial Guard will be disbanded and Burthorpe will come under control of the White Knights. We cannot let this happen!").also { stage = END_DIALOGUE }
                17 -> npcl(FacialExpression.FRIENDLY, "God speed friend!").also { stage = END_DIALOGUE }
            }
        }
