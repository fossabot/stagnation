 ...
 */
@Initializable
class Biohazard : Quest("Biohazard", 36, 35, 3,Vars.VARP_QUEST_BIOHAZARD_68,0,1,16) {

    companion object {
        const val ATTRIBUTE_FEED_ON_FENCE = "/save:biohazard:fed-fence"
    ...
    override fun drawJournal(player: Player?, stage: Int) {
        ...
        if(stage == 0) {
            ...
            line(player, "!!Completion of Plague City??", line++, isQuestComplete(player, "Plague City"))
        } else {
            if(stage > 0) {
            ...
            if(stage == 80) {
                ...
                line(player, "!!Elena?? asked me to take some items to !!Guidor?? in !!Varrock??. I", line++)
                line(player, "was warned that carrying some of these items into !!Varrock??", line++)
                line(player, "might be problematic so I gave them to the !!Chemist's??", line++)
                line(player, "!!Errand Boys?? to smuggle in. I should meet them at the", line++)
                line(player, "!!Dancing Donkey Inn?? in south east !!Varrock??.", line++)
            }
    ...
    override fun finish(player: Player?) {
        ...
        sendItemZoomOnInterface(player, Items.DISTILLATOR_420, 230, Components.QUEST_COMPLETE_SCROLL_277, 5)
