...
import content.quest.member.fightarena.cutscenes.EscapeCutscene
import content.quest.member.fightarena.dialogue.LazyGuardFADialogue
...
import content.quest.member.fightarena.npc.GeneralNPC
import core.api.*
...
/**
 * All interactions for Fight Arena quest.
 */
class FightArenaListeners : InteractionListener {
    ...
    override fun defineListeners() {
        ...
        on(GENERAL, IntType.NPC, "talk-to") { player, npc ->
            openDialogue(player, GenKhazardFADialogue(), npc)
            return@on true
        }

        on(HENGARD, IntType.NPC, "talk-to") { player, npc ->
            openDialogue(player, HengradFADialogue(), npc)
            return@on true
        }

        on(A_LAZY_GUARD_1, SCENERY, "talk-to") { player, node ->
            openDialogue(player, LazyGuardFADialogue(), node)
            return@on true
        }
