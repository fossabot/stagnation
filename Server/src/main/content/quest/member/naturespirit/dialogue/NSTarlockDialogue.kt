package content.quest.member.naturespirit.dialogue
...
/**
 * Represents the Tarlock dialogue plugin for Nature Spirit quest.
 */
@Initializable
class NSTarlockDialogue(player: Player? = null) : DialoguePlugin(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when(stage){
            ...
            } else sendDialogue("You give the journal to Filliman Tarlock").also { removeItem(
                player,
                Items.JOURNAL_2967,
                Container.INVENTORY
            ); stage = 10; setQuest(30) }

            //no journal
            8 -> npcl(FacialExpression.HALF_GUILTY, "Well, if I knew that, I wouldn't still be looking for it. However, I do remember something about a knot? Perhaps I was meant to tie a knot or something?").also { stage =
                END_DIALOGUE
            }
            ...
            55 -> sendDialogue(*splitLines("The druid produces a small sheet of papyrus with some writing on it.")).also { addItemOrDrop(
                player,
            ...
            58 -> npcl(FacialExpression.NEUTRAL, "It is required if you're to cast this druid spell. Once you've cast the spell, you should find something from nature. Bring it back to me and then we'll try to figure out the other things we need.").also { stage =
                END_DIALOGUE
            }
            ...
            64 -> sendDialogue("The spirit of Filliman Tarlock gives you another bloom spell.").also { addItemOrDrop(
                player,
                Items.DRUIDIC_SPELL_2968
            ); stage = END_DIALOGUE
            }
            ...
            75 -> sendDialogue("The spirit of Filliman Tarlock gives you","another bloom spell.").also { addItem(
                player,
                Items.DRUIDIC_SPELL_2968
            ); stage = END_DIALOGUE
            }
            ...
            130 -> if(inInventory(player, Items.DRUIDIC_SPELL_2968) || inBank(player, Items.DRUIDIC_SPELL_2968)){
                npcl(FacialExpression.NEUTRAL, "No, you've already got one!").also { stage =
                    END_DIALOGUE
                }
            } else {
