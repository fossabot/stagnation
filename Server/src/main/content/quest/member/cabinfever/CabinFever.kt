package content.quest.member.cabinfever
...
class CabinFever : Quest("Cabin Fever", 37, 36, 2,Vars.VARP_QUEST_CABIN_FEVER_655,0,1,131) {
    ...
    companion object {
        const val CabinFever = "Cabin Fever"
    }
    ...
    override fun drawJournal(player: Player?, stage: Int) {
        ...
        if (stage == 0) {
            ...
            line(player, "!!I must have completed Pirate's Treasure??", line++, isQuestComplete(player,"Pirate's Treasure"))
            line(player, if (hasLevelStat(player, Skills.AGILITY, 42)) "---42 Agility/--" else "!!42 Agility??", line++)
            line(player, if (hasLevelStat(player, Skills.CRAFTING, 45)) "---45 Crafting/--" else "!!45 Crafting??", line++)
            line(player, if (hasLevelStat(player, Skills.SMITHING, 50)) "---50 Smithing/--" else "!!50 Smithing??", line++)
