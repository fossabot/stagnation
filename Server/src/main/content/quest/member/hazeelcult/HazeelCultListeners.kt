package content.quest.member.hazeelcult

import config.Items
import config.NPCs
import config.Scenery
import content.quest.member.hazeelcult.npc.AlomoneNPC
import core.api.*
...
/**
 * Listeners for Hazeel Cult quest.
 */
class HazeelCultListeners : InteractionListener {
    companion object {
        ...
        const val SCROLL_INTERFACE = 222
    ...
    override fun defineListeners() {
        ...
        on(SEWER_1, IntType.SCENERY, "turn-left") { player, node ->
            ...
            if (getQuestStage(player, HazeelCult.questName) >= 1 && getAttribute(player, SEWER_LEFT, 0) == 0) {
                setAttribute(player, SEWER_LEFT, 1)
            }
            if (getQuestStage(player, HazeelCult.questName) == 10 && getAttribute(player, SEWER_RIGHT_2, 0) == 1) {
        ...
        on(SEWERS, IntType.SCENERY, "turn-right") { player, node ->
            ...
            } else if (getQuestStage(player, HazeelCult.questName) == 10) {
                if (!player.location.withinDistance(node.asScenery().location, 2)) return@on true
                for (i in SEWERS) when (i) {
        ...
        onUseWith(IntType.SCENERY, POISON, COOKING_RANGE) { player, used, with ->
            if (getAttribute(player, MAHJARRAT, true) && removeItem(player, POISON)) {
                ...
                setQuestStage(player, HazeelCult.questName, 3)
            } else {
                sendNPCDialogue(
                    player,
        ...
        on(CRATE, IntType.SCENERY, "search") { player, node ->
            if (getQuestStage(player, HazeelCult.questName) >= 1 && getAttribute(player, MAHJARRAT, true) && !getAttribute(player, CARNILLEAN, true) && freeSlots(player) > 1) {
        ...
        on(Scenery.CHEST_2857, IntType.SCENERY, "Close") { player, _ ->
        ...
        fun hazeelScroll(player: Player) {
            ...
            setInterfaceText(player, scroll.joinToString("<br>"), 222, 2)
        }
