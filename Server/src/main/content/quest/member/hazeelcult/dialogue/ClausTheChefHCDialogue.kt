...
/**
 * Represents the Claus The Chef dialogue plugin for Hazeel Cult quest.
 */
@Initializable
class ClausTheChefHCDialogue(player: Player? = null) : DialoguePlugin(player) {
    override fun handle(componentID: Int, buttonID: Int): Boolean {
        ...
        when {
            ...
            (questStage == 3) -> when(stage){
                0 -> if(getAttribute(player, HazeelCultListeners.MAHJARRAT, true)) {
            ...
            (questStage == 100) -> when(stage){
                0 -> if(getAttribute(player, HazeelCultListeners.MAHJARRAT, true)&& !getAttribute(player, HazeelCultListeners.CARNILLEAN, true)){
