...
/**
 * Represents the David dialogue plugin for Murder Mystery quest.
 */
@Initializable
class DavidMMDialogue(player: Player? = null) : DialoguePlugin(player) {
    ...
    override fun handle(componentID: Int, buttonID: Int): Boolean {
        when (getQuestStage(player!!, MurderMystery.MurderMystery)) {
            in 1..3 -> when (stage) {
                1 -> npcl(FacialExpression.ANNOYED, "And? Make this quick. I have better things to do than be interrogated by halfwits all day.").also { stage = 2 }
                ...
                7 -> playerl(FacialExpression.SUSPICIOUS, "So you didn't hear any sounds of a struggle or any barking from the guard dog next to his study window?").also { stage = 8 }
                ...
                9 -> npcl(FacialExpression.NEUTRAL, "There was a nest of spiders upstairs between the two servants' quarters. Obviously I had to kill them before our pathetic servants whined at my father some more.").also { stage = 10 }
                10 -> npcl(FacialExpression.NEUTRAL, "Honestly, it's like they expect to be treated like royalty! If I had my way I would fire the whole workshy lot of them!").also { stage = 11 }
                11 -> {
                    end()
