...
/**
 * Represents the Carol dialogue plugin for Murder Mystery quest.
 */
@Initializable
class CarolMMDialogue(player: Player? = null) : DialoguePlugin(player) {
    ...
    override fun handle(interfaceId: Int, buttonId: Int): Boolean {
        when (getQuestStage(player!!, MurderMystery.MurderMystery)) {
            in 1..3 -> when (stage) {
                1 -> npcl(FacialExpression.ANNOYED, "Well, ask what you want to know then.").also { stage = 2 }
                ...
                8 -> npcl(FacialExpression.ANNOYED, "blocked, and as nobody else here has the intelligence to even unblock a simple drain I felt I had to do it myself.").also { stage = 9 }
                9 -> {
                    end()
            ...
            100 -> when (stage) {
                1 -> npcl(FacialExpression.NEUTRAL, "Apparently you aren't as stupid as you look.").also { stage = END_DIALOGUE }
            }
        }
        return true
    }
