...
/**
 * Represents the Pierre dialogue plugin for Murder Mystery quest.
 */
@Initializable
class PierreMMDialogue(player: Player? = null) : DialoguePlugin(player) {
    ...
    override fun handle(componentID: Int, buttonID: Int): Boolean {
        when (getQuestStage(player!!, MurderMystery.MurderMystery)) {
            in 1..3 -> when (stage) {
                ...
                6 -> npcl(FacialExpression.NEUTRAL, "Well, like what?").also { stage = 7 }
                7 -> playerl(FacialExpression.ASKING, "Any sounds of a struggle with Lord Sinclair?").also { stage = 8 }
                8 -> npcl(FacialExpression.NEUTRAL, "No, I don't remember hearing anything like that.").also { stage = 9 }
                ...
                11 -> npcl(FacialExpression.NEUTRAL, "Well, I know David said that he was going to do something about the spiders' nest that's between the two servants' quarters upstairs.").also { stage = 12 }
