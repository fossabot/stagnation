...
/**
 * Represents the Hobbes dialogue plugin for Murder Mystery quest.
 */
@Initializable
class HobbesMMDialogue(player: Player? = null) : DialoguePlugin(player) {
    ...
    override fun handle(componentID: Int, buttonID: Int): Boolean {
        when (getQuestStage(player!!, MurderMystery.MurderMystery)) {
            1 -> when (stage) {
                ...
                4 -> npcl(FacialExpression.NEUTRAL, "Well, in my considered opinion it must be David. The man is nothing more than a bully. And I happen to know that poor Lord Sinclair and David had a massive argument in the living room about the way he treats the staff, the other day.").also { stage = 5 }
                ...
                7 -> npcl(FacialExpression.ASKING, "How do you mean 'suspicious'?").also { stage = 8 }
                8 -> playerl(FacialExpression.ASKING, "Any sounds of a struggle with Lord Sinclair?").also { stage = 9 }
                9 -> npcl(FacialExpression.NEUTRAL, "No, I definitely didn't hear anything like that.").also { stage = 10 }
                10 -> playerl(FacialExpression.ASKING, "How about the guard dog barking at all?").also { stage = 11 }
                ...
                12 -> npcl(FacialExpression.NEUTRAL, "Well, I do know that Elizabeth was extremely annoyed by the mosquito nest under the fountain in the garden, and was going to do something about it. I suspect any poison she bought would have been").also { stage = 13 }
                13 -> npcl(FacialExpression.NEUTRAL, "enough to get rid of it. A Good job too, I hate mosquitos.").also { stage = 14 }
                14 -> playerl(FacialExpression.NEUTRAL, "Yeah, so do I.").also { stage = 15 }
