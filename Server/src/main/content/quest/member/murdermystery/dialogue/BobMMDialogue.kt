...
/**
 * Represents the Bob dialogue plugin for Murder Mystery quest.
 */
@Initializable
class BobMMDialogue(player: Player? = null) : DialoguePlugin(player) {
    ...
    override fun handle(componentID: Int, buttonID: Int): Boolean {
        when (getQuestStage(player!!, MurderMystery.MurderMystery)) {
            in 1..3 -> when (stage) {
                1 -> npcl(FacialExpression.HALF_GUILTY, "I suppose I had better talk to you then.").also { stage = 2 }
                ...
                10 -> npcl(FacialExpression.ANGRY, "If you absolutely must know, we had a problem with the beehive in the garden, and as all of our servants are so pathetically useless, I decided I would deal with it myself. So I did.").also { stage = END_DIALOGUE }
                11 -> {
                    end()
            ...
            100 -> when (stage) {
                1 -> npcl(FacialExpression.HALF_GUILTY, "Apparently you aren't as stupid as you look.").also { stage = END_DIALOGUE }
            }
        }
        return true
    }
