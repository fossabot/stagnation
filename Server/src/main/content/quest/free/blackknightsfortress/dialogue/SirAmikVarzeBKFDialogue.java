...
import content.quest.free.blackknightsfortress.BlackKnightsFortress;
import content.quest.member.recruitmentdrive.npc.SirAmikVarzeAdditionalDialogue;
...
/**
 * Represents the Sir Amik Varze dialogue plugin for Black Knight's Fortress quest.
 */
public class SirAmikVarzeBKFDialogue extends DialoguePlugin {
	...
	/**
	 * Constructs a new {@code SirAmikVarzeBKFDialogue} {@code Object}.
	 */
	public SirAmikVarzeBKFDialogue() {
		/**
		 * empty.
		 */
	}
	 ...
	 */
	public SirAmikVarzeBKFDialogue(Player player) {
		super(player);
	}

	@Override
	public DialoguePlugin newInstance(Player player) {
		return new SirAmikVarzeBKFDialogue(player);
	}
	...
	public boolean open(Object... args) {
		...
		case 20:
			...
			break;
		case 100:
			if (player.getQuestRepository().getQuest("Recruitment Drive").getStage(player) == 10) {
				openDialogue(player, new SirAmikVarzeAdditionalDialogue(), npc);
			} else {
	...
	public boolean handle(int interfaceId, int buttonId) {
		...
		case 100:
			...
			case 1:
				...
				break;
			case 2:
				if (player.getQuestRepository().getQuest("Recruitment Drive").isStarted(player) && (player.getQuestRepository().getQuest("Black Knights' Fortress").isCompleted(player))) {
					interpreter.sendDialogues(npc, FacialExpression.HALF_THINKING, "Quests, eh?", "Well, I don't have anything on the go at the moment.");
					stage = 7;
				} else if (player.getQuestRepository().getQuest("Recruitment Drive").isCompleted(player)) {
					interpreter.sendDialogues(npc, FacialExpression.HALF_THINKING, "Quests, eh?", "Well, I don't have anything on the go at the moment.");
					stage = 7;
				} else {
					interpreter.sendDialogues(npc, FacialExpression.NEUTRAL, "Quests, eh?", "Well, I don't have anything on the go at the moment,", "but there is an organisation that is always looking for", "capable adventurers to assist them.");
					stage = 3;
				}
				break;
			case 3:
			...
			case 4:
				...
				break;
				case 5:
					if (player.getQuestRepository().isComplete("Druidic Ritual") && (player.getQuestRepository().isComplete("Black Knights' Fortress"))) {
						interpreter.sendOptions("Select an Option", "Yes, please. Sign me up!", "No, thanks");
						stage = 6;
					} else {
						interpreter.sendDialogues(player, FacialExpression.HALF_ASKING, "No, thanks");
						stage = 7;
					}
					break;
				case 6:
					switch (buttonId) {
						case 1:
							interpreter.sendDialogues(player, FacialExpression.HALF_ASKING, "Yes, please. Sign me up!");
							openDialogue(player, new SirAmikVarzeAdditionalDialogue(), npc);
							stage = 5;
							break;
						case 2:
