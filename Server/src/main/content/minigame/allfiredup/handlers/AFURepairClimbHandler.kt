...
import config.Items
import content.global.skill.member.construction.NailType
...
class AFURepairClimbHandler : InteractionListener {
    ...
    private fun repair(player: Player,rco: RepairClimbObject){
        ...
        if(requiresNeedle){
            if(inInventory(player, Items.NEEDLE_1733) && player.inventory.containItems(*requiredItems.map { it.id }.toIntArray())) {
            ...
            if(inInventory(player, Items.HAMMER_2347) && player.inventory.containItems(*requiredItems.map { it.id }.toIntArray())) {
