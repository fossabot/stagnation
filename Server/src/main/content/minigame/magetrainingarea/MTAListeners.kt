...
class MTASpellListeners : SpellListener("modern"){
    ...
    private val MTA_ALCH_ITEMS = content.minigame.magetrainingarea.impl.AlchemistZone.AlchemistItem.values().map{it.item.id}.toIntArray()
    ...
    override fun defineListeners() {
        ...
        onCast(Modern.HIGH_ALCHEMY,ITEM,*MTA_ALCH_ITEMS){ p, node ->
            ...
            requires(p,55)
            val alchItem = content.minigame.magetrainingarea.impl.AlchemistZone.AlchemistItem.forItem(item.id) ?: return@onCast
            val coins = Item(content.minigame.magetrainingarea.impl.AlchemistZone.COINS.id, alchItem.cost)
            val freeAlch = alchItem == content.minigame.magetrainingarea.impl.AlchemistZone.freeConvert

            if (p.inventory.getAmount(content.minigame.magetrainingarea.impl.AlchemistZone.COINS.id) + alchItem.cost > 10000) {
