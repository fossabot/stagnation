...
import content.global.skill.member.slayer.Tasks
...
fun hasRequirement(player: Player, req: QuestReq, message: Boolean = true) : Boolean {
    ...
    if (unmetReqs.size == 2 && unmetReqs[0] is QuestReq) {
        messageList.add (DARK_RED + "You must complete ${(unmetReqs[0] as QuestReq).questReq.questName} to use this.")
    } else {
        messageList.add ("Nothing interesting happens.")
    }
