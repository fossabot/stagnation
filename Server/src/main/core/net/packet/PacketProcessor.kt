...
object PacketProcessor {
    ...
    private fun process(pkt: Packet) {
        when (pkt) {
            ...
            is Packet.ContinueOption -> {
                ...
                player.debug("interface: <col=0000ff>${pkt.iface}</col>")
                player.debug("child: <col=0000ff>${pkt.child}</col>")
                player.debug("slot: <col=0000ff>${pkt.slot}</col>")
                player.debug("------------------------")
            ...
            is Packet.ItemExamine -> {
                ...
                pkt.player.debug("itemid: [<col=0000ff>${pkt.id}</col>] value: [<col=0000ff>${def.value}</col>]")
            ...
            is Packet.SceneryExamine -> {
                ...
                pkt.player.debug("id: [<col=0000ff>${pkt.id}</col>]")
                if (def.configFile != null)
                    pkt.player.debug("varbit: [<col=0000ff>${def.configFile.id}</col>]")
                pkt.player.debug("------------------------------")
            ...
            is Packet.NpcExamine -> {
                ...
                pkt.player.debug("npc_id: <col=0000ff>${pkt.id}</col>")
                if (def.configFileId != -1)
                    pkt.player.debug("varbit: <col=0000ff>${def.configFileId}</col>")
                pkt.player.sendMessage(def.examine)
            }
    ...
    private fun processSlotSwitch(pkt: Packet) {
        ...
        //TODO See above TODO, This was hardcoded in the packet decoder.
    ...
    private fun processCloseIface(pkt: Packet.CloseIface) {
        ...
        if (player.getAttribute<Any?>("worldMap:viewing") != null) {
            removeAttribute(player,"worldMap:viewing")
    ...
    private fun processIfAction(pkt: Packet.IfAction) {
        ...
        player.debug("interface: [<col=0000ff>${pkt.iface}</col>], btn: [<col=0000ff>${pkt.child}</col>]")
        player.debug("slot: [<col=0000ff>${pkt.slot}</col>], itemid: [<col=0000ff>${pkt.itemId}</col>]")
        player.debug("optindex: [<col=0000ff>${pkt.optIndex}</col>], op: [<col=0000ff>${pkt.opcode}</col>]")
        player.debug("-------------------------------------")
    ...
    private fun processSceneryAction(pkt: Packet.SceneryAction) {
        ...
        if (scenery == null || scenery.id != objId || !scenery.isActive) {
            player.debug("[SCENERY INTERACT] NULL OR ????")
        ...
        if (option == null) {
            player.debug("[SCENERY INTERACT] ???? OPTION")
            InteractPlugin.handleInvalidInteraction(player, scenery, Option.NULL)
            return sendClearMinimap(player)
        }
        ...
        player.debug("id: [<col=0000ff>${wrapperChild.id}</col>], opt: [<col=0000ff>${option.name}</col>] [<col=0000ff>${option.index}</col>]")
        player.debug("location: <col=0000ff>${scenery.location}</col>, direction: [<col=0000ff>${scenery.direction}</col>]")
        if (hasWrapper) {
            player.debug("wrapper: [<col=0000ff>${scenery.id}</col>], ${scenery.definition.configFile?.let { "varbit: [<col=0000ff>${it.id}</col>]"} ?: "varp: [<col=0000ff>${scenery.definition.configId}</col>]"}")
        }
        player.debug("------------------------------------------------")
    ...
    private fun processNpcAction(pkt: Packet.NpcAction) {
        ...
        pkt.player.debug("npcid: [<col=0000ff>${wrapperChild.id}</col>], index: [<col=0000ff>${pkt.npcIndex}</col>]")
        pkt.player.debug("opt: [<col=0000ff>${option.name}</col>] - [<col=0000ff>${option.index}</col>]")
        pkt.player.debug("spawn: <col=0000ff>${npc.properties.spawnLocation}</col>")
        if (hasWrapper) {
            pkt.player.debug("wrapper: [<col=0000ff>${npc.id}</col>], varbit: [<col=0000ff>${npc.definition.configFileId}</col>]")
        }
        pkt.player.debug("---------------------------------")
    ...
    private fun processItemAction(pkt: Packet.ItemAction) {
        ...
        pkt.player.scripts.reset()
        pkt.player.debug("itemid: [<col=0000ff>${item.id}</col>], slot: [<col=0000ff>${pkt.slot}</col>], option: [<col=0000ff>${option.name}</col>]")
    }
