...
import config.Items
import content.global.activity.dailytasks.JobManager
...
class DevelopmentCommandSet : CommandSet(Privilege.ADMIN) {
    ...
    override fun defineCommands() {
         ...
         */
        define("farmkit", Privilege.ADMIN, "::farmkit", "Provides a kit of various farming equipment."){player,_ ->
            for(item in farmKitItems){
                player.inventory.add(Item(item))
            }
        }

        define("enl", Privilege.ADMIN) { player, args ->
        ...
        define("killme", Privilege.ADMIN, "::killme", "Does exactly what it says on the tin.") { player, _ ->
        ...
        define("dumpstructs", Privilege.ADMIN, "::dumpstructs", "Dumps all the cache structs to structs.txt") {player, _ ->
        ...
        define("dumpdatamaps", Privilege.ADMIN, "::dumpdatamaps", "Dumps all the cache data maps to datamaps.txt") {player, _ ->
        ...
        define("npcsearch", Privilege.ADMIN, "npcsearch name", "Searches for NPCs that match the name either in main or children.") {player, strings ->
        ...
        define("runekit", Privilege.ADMIN, "::runekit", "Gives 1k of each Rune type" ) { player, args ->
                    for(item in runeKitItems) {
                    addItem(player, item, 1000)
            }
        }

        define("drawchunks", Privilege.ADMIN, "::drawchunks", "Draws the border of the chunk you're standing in") {player, _ ->
            setAttribute (player, "chunkdraw", !getAttribute(player, "chunkdraw", false))
        }

        define("drawregions", Privilege.ADMIN, "::drawregions", "DRaws the border of the region you're standing in") {player, _ ->
            setAttribute (player, "regiondraw", !getAttribute(player, "regiondraw", false))
        }

        define("drawroute", Privilege.ADMIN, "::drawroute", "Visualizes the path your player is taking") {player, _ ->
            setAttribute (player, "routedraw", !getAttribute(player, "routedraw", false))
        }

        define ("fmstart", Privilege.ADMIN, "::fmstart", "") {player, _ ->
            setAttribute(player, "fmstart", Location.create(player.location))
        }

        define ("fmend", Privilege.ADMIN, ":fmend", "") {player, _ ->
            setAttribute(player, "fmend", Location.create(player.location))
        }

        define ("fmspeed", Privilege.ADMIN, "::fmspeed", "") {player, args ->
            setAttribute(player, "fmspeed", args[1].toIntOrNull() ?: 10)
        }

        define ("fmspeedend", Privilege.ADMIN, "::fmspeedend", "") {player, args ->
            setAttribute(player, "fmspeedend", args[1].toIntOrNull() ?: 10)
        }

        define("testfm", Privilege.ADMIN, "::testfm", "test ForveMove") { player, _ ->
        ...
        define("fmanim", Privilege.ADMIN, "::fmanim", "") {player, args ->
