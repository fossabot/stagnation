...
import content.global.activity.treasuretrail.TreasureTrailManager
        ...
        private fun display(player: Player, pageNum: Int, buttonId: Int) : Boolean {
            ...
            for(i in (68..97)) {
                when(pageNum) {
                    0 -> {
                        when(i) {
                            97 -> sendLine(player, SPACER,i)
                            68 -> sendLine(player,"Easy clues: ${TreasureTrailManager.getInstance(queryPlayer).completedClues[0]}",i)
                            69 -> sendLine(player,"Medium clues: ${TreasureTrailManager.getInstance(queryPlayer).completedClues[1]}",i)
                            70 -> sendLine(player,"Hard clues: ${TreasureTrailManager.getInstance(queryPlayer).completedClues[2]}",i)
                            71 -> sendLine(player, SPACER,i)
                            72 -> sendLine(player,"Slayer tasks: ${SlayerManager.getInstance(queryPlayer).flags.completedTasks}",i)
                            73 -> sendLine(player,"Quest points: ${queryPlayer.questRepository.points}",i)
                            74 -> sendLine(player,"Ironman mode: ${queryPlayer.ironmanManager.mode.name.lowercase(Locale.getDefault())}",i)
                            75 -> sendLine(player,"Deaths: ${queryPlayer.getAttribute("$STATS_BASE:$STATS_DEATHS",0)}",i)
                            76 -> sendLine(player, SPACER,i)
                            77 -> sendLine(player,"Logs chopped: ${queryPlayer.getAttribute("$STATS_BASE:$STATS_LOGS",0)}",i)
                            78 -> sendLine(player,"Rocks mined: ${queryPlayer.getAttribute("$STATS_BASE:$STATS_ROCKS",0)}",i)
                            79 -> sendLine(player,"Fishes caught: ${queryPlayer.getAttribute("$STATS_BASE:$STATS_FISH",0)}",i)
                            80 -> sendLine(player, "Essences crafted: ${queryPlayer.getAttribute("$STATS_BASE:$STATS_RC",0)}", i)
                            81 -> sendLine(player, SPACER,i)
