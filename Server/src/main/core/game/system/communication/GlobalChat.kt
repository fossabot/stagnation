...
class GlobalChat : Commands {
    override fun defineCommands() {
        define("muteglobal", Privilege.ADMIN, "", "Toggles global chat on or off.") {player, _ ->
    ...
    companion object {
        ...
        private fun prepare(sender: String, message: String, isResizable: Boolean, rights: Int): String {
            val baseColor = if (isResizable) "%ffffff" else "%000000"
            val bracketColor = if (isResizable) "%ffffff" else "%000000"
            return colorize("$bracketColor[${baseColor}Global$bracketColor] ${if (rights > 0) "<img=${rights - 1}>" else ""}$sender: ${baseColor}$message")
        }
    }
}
