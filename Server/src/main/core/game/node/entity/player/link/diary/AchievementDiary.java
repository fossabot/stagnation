...
public class AchievementDiary {
	...
	public void updateTask(Player player, int level, int index, boolean complete) {
		...
		if (isComplete(level)) {
			player.getDialogueInterpreter().sendDialogue("Congratulations! You have completed all of the " + getLevel(level).toLowerCase()
					+ ""," tasks in the " + type.getName() + " area. Speak to " + NPCDefinition.forId(type.getNpc(level)).getName() + " to claim your","reward.");
		}
		drawStatus(player);
	}
